...
def gstreamer_pipeline():
    return (
        ...
        "videoconvert ! video/x-raw, format=BGR ! appsink drop=1 sync=false max-buffers=1"
    )
...
if not cap.isOpened():
    ...
    print("Test pipeline manually with:")
    print(gstreamer_pipeline().replace(
        "appsink drop=1 sync=false max-buffers=1", "autovideosink sync=false"))
    exit()
...
try:
    while True:
        ...
        cv2.imshow("YOLO11 Person Inspection (RTSP)", frame)
